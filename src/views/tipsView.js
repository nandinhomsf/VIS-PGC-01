...
export function renderTipsView(data) {
  ...
  const y = d3.scaleLinear().domain([0, d3.max(data.tips, (d) => d.avg_tip)]).nice().range([height, 0]);
  const color = d3.scaleOrdinal().domain(regions).range(regions.map((r) => REGION_COLORS[r]));
  const line = d3.line().x((d) => x(d.dayLabel)).y((d) => y(d.avg_tip));
  ...
  d3.groups(data.tips, (d) => d.region).forEach(([region, values]) => {
    ...
    g.selectAll(`.tip-${region}`).data(sorted).join('circle').attr('cx', (d) => x(d.dayLabel)).attr('cy', (d) => y(d.avg_tip)).attr('r', 4).attr('fill', color(region));
  });
